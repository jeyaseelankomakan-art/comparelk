...
function extractBrandFromTitle(string $title): ?string {
    ...
    $brands = [
        'Samsung', 'Apple', 'LG', 'Sony', 'Panasonic', 'Abans', 'Philips',
        'Hisense', 'Singer', 'Nokia', 'Xiaomi', 'Oppo', 'Vivo', 'Huawei',
        'Haier', 'Midea', 'Beko', 'Bosch', 'Sharp', 'TCL', 'Whirlpool',
        'Electrolux', 'Motorola', 'Realme', 'Infinix', 'Tecno', 'Itel',
        'Lenovo', 'Honor',
    ];
...
// Uses cURL so timeouts are actually enforced (file_get_contents is unreliable on Windows).
...
function downloadAndSaveImage(string $url): ?string {
    ...
    // Use cURL with enforced timeouts instead of file_get_contents
    $ch = curl_init($url);
...
function scrapeCategoryPage(PDO $pdo, int $storeId, string $url, string $parserClass, ?int $categoryId = null): array {
    ...
    if ($html === false || !empty($curlErr) || $httpCode >= 400) {
        $result['message'] = "Could not fetch URL: $url (HTTP $httpCode" . (!empty($curlErr) ? ", $curlErr" : "") . ")";
        return $result;
    }
...
class KaprukaCategoryParser implements CategoryParserInterface {
    ...
    public function parseHtml(string $html, string $baseUrl): array {
        ...
        foreach ($cards as $card) {
            ...
            // We collect all child <div> elements and pick the one with the most text.
...
class BuyabansParser implements CategoryParserInterface {
    ...
    // -------------------------------------------------------------------------
    // Extract the category ID from the page HTML or URL.
    // BuyAbans embeds it in the JS initialisation code, e.g.:
    //   category_id: 15,
    // or it may appear as a URL query parameter.
    // -------------------------------------------------------------------------
...
if (!$price || $price <= 0) continue; // --- Image --- $imgNode=$xpath->query('.//*[contains(@class,
    "grid-product-img")] | .//img', $card)->item(0);
    $imgUrl = '';
    if ($imgNode) {
    ...
    $brand = null;
    $knownBrands = [
    'Samsung', 'Apple', 'LG', 'Sony', 'Panasonic', 'Philips', 'Hisense',
